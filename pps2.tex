...
para la mantenibilidad del código. La dedicación a la etapa de testing en un proyecto de software representa aproximadamente el 50\% del esfuerzo total.
Esto muestra la importancia y tiempo que demanda para ofrecer código de calidad. Herramientas como JUnit han facilitado la generación de casos de test
por parte de los programadores. Sin embargo, su elaboración sigue siendo manual y, por lo tanto, costosa.
...
Se han mencionado ciertas limitaciones propias de la herramienta desarrollada. El objetivo a corto plazo es superar algunas de estas.
Actualmente sólo puede generar test automáticamente para métodos cuyos parámetros y restricciones en el código sean de tipo entero, además de soportar
las operaciones aritméticas básicas (+, -, *, /, \%). Se pretende extenderlo a los tipos de datos y operaciones aritméticas soportadas por el SMT Solver Z3.
El objetivo a mediano plazo es extender la herramienta para soportar la generación de test sobre programas que realizan llamadas a funciones dentro del código.
